...
class PiRacerSteering:
    def __init__(self, piracer):
        self.piracer = piracer
    def run(self, angle):
        self.piracer.set_steering_percent(angle+0.72)
        return angle

class PiRacerThrottle:
    def __init__(self, piracer):
        self.piracer = piracer
...
####################################################################
    ...
    elif cfg.DRIVE_TRAIN_TYPE == "CUSTOM_DRIVING_MODE":

        piracer = PiRacerStandard()
        V.add(PiRacerSteering(piracer), inputs=['angle'])
        V.add(PiRacerThrottle(piracer), inputs=['throttle'])
